    ...
    (() => {
        ...
        class Popup {
            ...
            open(selectorValue) {
                if (bodyLockStatus) {
                    ...
                    } else this.popupLogging(`Ой, такого попапу нет. Проверьте корректность ввода. `);
                }
            }
            close(selectorValue) {
        ...
        function initSliders() {
            if (document.querySelector(".about__slider")) {
                ...
                function aboutSliderFix() {
                    const aboutContainer = document.querySelector(".about__container");
                    const diff = (window.innerWidth - aboutContainer.offsetWidth) / 2;
                    if (diff > 0) document.querySelector(".about__slider").style.width = document.querySelector(".about__body-slider").offsetWidth + diff + 15 + "px"; else document.querySelector(".about__slider").style.width = document.querySelector(".about__body-slider").offsetWidth + 15 + "px";
                }
                window.addEventListener("resize", aboutSliderFix);
                aboutSliderFix();
            }
            if (document.querySelector(".reviews__slider")) {
                ...
                function reviewsSliderFix() {
                    const reviewsContainer = document.querySelector(".reviews__container");
                    const diff = (window.innerWidth - reviewsContainer.offsetWidth) / 2;
                    if (diff > 0) document.querySelector(".reviews__slider").style.width = document.querySelector(".reviews__body").offsetWidth + diff + 15 + "px"; else document.querySelector(".reviews__slider").style.width = document.querySelector(".reviews__body").offsetWidth + 15 + "px";
                }
                window.addEventListener("resize", reviewsSliderFix);
                reviewsSliderFix();
            }
        }
...
***************************************************************************** */
        ...
        function parallaxImages() {
            const parallax = document.querySelector(".about__images-left");
            if (parallax) {
                const parallaxContainer = document.querySelector(".parallax-container");
                const parallaxElement = document.querySelector(".parallax-element");
                const containerSpeed = .1;
                const elementSpeed = 0;
                window.addEventListener("scroll", (() => {
                    const containerTop = parallaxContainer.getBoundingClientRect().top;
                    const elementTop = containerTop * elementSpeed - window.scrollY * elementSpeed;
                    parallaxElement.style.transform = `translateY(${elementTop}px)`;
                    parallaxContainer.style.transform = `translateY(${containerTop * containerSpeed}px)`;
                }));
            }
        }
        const mediaQuery = window.matchMedia("(min-width: 991.98px)");
        function handleMediaChange(event) {
            if (event.matches) parallaxImages(); else console.log("Функция отключена.");
        }
